...
- 고정 감정 태그 5개는 `차분`, `기쁨`, `피곤`, `포근`, `바쁨`으로 확정한다.
- 앱 삭제 시 데이터가 함께 삭제될 수 있다는 점은 Settings local trust/backup area에서 안내한다.
- 백업 파일 포맷은 MVP v1 zip package로 한다: `manifest.json`, originals, thumbnails.
- 전체 데이터 삭제는 로컬 기록과 app-managed entry assets만 삭제하고 onboarding completion/app preferences는 유지한다.
